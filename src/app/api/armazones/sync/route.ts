...
async function sincronizarArmazonesPublico(items: ItemLocal[]) {
  ...
  // Columna para ocultar lo que ya no está en el inventario sin borrar filas
  // (ver sincronizacionArmazones.ts). Se crea la primera vez que hace falta.
  const columna = await pool.query(
    `SELECT 1 FROM information_schema.columns
     WHERE table_name = 'armazones_publico' AND column_name = 'en_inventario'`
  );
  if (columna.rowCount === 0) {
    await pool.query(
      'ALTER TABLE armazones_publico ADD COLUMN IF NOT EXISTS en_inventario boolean NOT NULL DEFAULT true'
    );
  }
  ...
  try {
    ...
    const existentes = await client.query<FilaPublica>(
      'SELECT id, marca, modelo, categoria, precio, imagen_url, stock_visible, en_inventario FROM armazones_publico'
    );
    const plan = planificarArmazonesPublico(existentes.rows, items);
    bajasRetenidas = plan.bajasRetenidas;
    if (plan.bajasRetenidas > 0) {
      console.warn(`Armazones sync: inventario incompleto (${items.length}), no se ocultan ${plan.bajasRetenidas} filas`);
    }
    ...
    for (let i = 0; i < plan.nuevos.length; i += FILAS_POR_LOTE) {
      const lote = plan.nuevos.slice(i, i + FILAS_POR_LOTE);
      const valores: unknown[] = [];
      const placeholders = lote.map((armazon, idx) => {
        const base = idx * 7;
        valores.push(
          armazon.marca,
          armazon.modelo,
    ...
    if (plan.bajas.length > 0) {
      const resultado = await client.query(
        'UPDATE armazones_publico SET en_inventario=false WHERE id = ANY($1::int[])',
        [plan.bajas]
      );
      ocultados = resultado.rowCount ?? 0;
    }
